...
<?php
    ...
    $Typ = $_POST['Typ'];
    $TRITA_DL = $_POST['TRITA_DL'];
    $TRITA_nr_DL = $_POST['TRITA_nr_DL'];
    $TRITA_R = $_POST['TRITA_R'];
    $TRITA_nr_R = $_POST['TRITA_nr_R'];
    $Titel = $_POST['Titel'];
    $KTHid = $_POST['KTHid'];
    $Enamn = $_POST['Enamn'];
    $Fnamn = $_POST['Fnamn'];
    ...
    if (strlen($Typ) > 4 and strlen($Titel) > 5 and strlen($Enamn) > 0 and strlen($Fnamn) > 0 and strlen($Epost) > 5 and strlen($KTHid) == 8) {
 ...
 {
        echo 'The publication has already got an ISBN!';              
 }
   else 
 {
    ...
    // Hämta ISBN-nummer
		...
		try {
            ...
            if ($Typ == "Rapport") {
                		...
                		VALUES (trim(:TRITA_DL),trim(:TRITA_nr_DL),trim(:Epost),:KTHid,:Fnamn,:Enamn,:isbn,:Typ,:Titel,CURDATE())");
			   $stmt->bindParam(':TRITA_DL', $TRITA_DL);
	                   $stmt->bindParam(':TRITA_nr_DL', $TRITA_nr_DL);	
			}
	    else {
	                   $Typ = "Doktor/Lic";
                           $stmt = $pdo->prepare("INSERT INTO reg_isbn 
                		(TRITA,TRITA_nr,Epost,KTH_id,Fnamn,Enamn,ISBN,Pubtyp,Titel,Regdatum) 
                		VALUES (trim(:TRITA_R),trim(:TRITA_nr_R),trim(:Epost),:KTHid,:Fnamn,:Enamn,:isbn,:Typ,:Titel,CURDATE())");
			   $stmt->bindParam(':TRITA_R', $TRITA_R);
	                   $stmt->bindParam(':TRITA_nr_R', $TRITA_nr_R);	
			}
	...
	if ($trans_klar == 1) {
		echo '';
        echo 'Your request for ISBN is registrered: ' . $isbn;
		echo '';
		echo '';
	...
	$mail->SMTPSecure = "tls"; // NYTT 2022-11-22          
		...
		echo '';
        echo 'Error by ISBN request, please contact the PIMS unit at the University administration!'; 
	}
